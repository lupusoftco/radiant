...
$_GET["arama"] = escape_sql_string($_GET["arama"]);
...
?>
		<div class="home">
			<div class="home_container d-flex flex-column align-items-center justify-content-end">
				<div class="home_content text-center">
					<div class="home_title">Keşfet</div>
					<div class="breadcrumbs d-flex flex-column align-items-center justify-content-center">
						<ul class="d-flex flex-row align-items-start justify-content-start text-center">
							<li><a href="index.php">Anasayfa</a></li>
							<li>Keşfet</li>
							<li><a href="index.php?arama=<?php echo $_GET["arama"]; ?>"><?php echo $_GET["arama"]; ?></a></li>
						</ul>
					</div>
				</div>
			</div>
		...
		<div class="products">
			<div class="container">
						...
						<div class="products_bar d-flex flex-lg-row flex-column align-items-lg-center align-items-start justify-content-lg-start justify-content-center">
							<div class="products_bar_links">
								<ul class="d-flex flex-row align-items-start justify-content-start">
									<li><a href='?arama=<?php echo $_GET["arama"]; ?>'>Tümü</a></li>
									<li><a href='?arama=<?php echo $_GET["arama"]; ?>&where=popular'>En Popüler</a></li>
									<li><a href='?arama=<?php echo $_GET["arama"]; ?>&where=new'>Yeni Ürünler</a></li>
									<li><a href='?arama=<?php echo $_GET["arama"]; ?>&where=forgotten'>Keşfedilmemiş</a></li>
								</ul>
							</div>
							<div class="products_bar_side d-flex flex-row align-items-center justify-content-start ml-lg-auto">
								<div class="products_dropdown product_dropdown_sorting">
									...
									<ul>
										<li class="item_sorting_btn"><a href='?arama=<?php echo $_GET["arama"]; ?>'>Standart</a></li>
										<li class="item_sorting_btn"><a href='?arama=<?php echo $_GET["arama"]; ?>&orderby=asc'>Artan</a></li>
										<li class="item_sorting_btn"><a href='?arama=<?php echo $_GET["arama"]; ?>&orderby=desc'>Azalan</a></li>
									</ul>
								</div>
								<div class="products_dropdown text-right product_dropdown_filter">
									<div class="isotope_filter_text"><span>Filtre</span><i class="fa fa-caret-down" aria-hidden="true"></i></div>
									<ul>
										<li class="item_filter_btn"><a href='?arama=<?php echo $_GET["arama"]; ?>'>Tümü</li>
										<li class="item_filter_btn"><a href='?arama=<?php echo $_GET["arama"]; ?>&where=stok'>Kritik Stok</a></li>
				...
				<div class="row products_row products_container grid" id="products">
					<?php
		                ...
		                @$sorgu = mysqli_query($db,"SELECT COUNT(*) AS toplam FROM urunler WHERE (isim LIKE '%".$_GET["arama"]."%' or aciklama LIKE '%".$_GET["arama"]."%' or fiyat LIKE '%".$_GET["arama"]."%' or etiketler LIKE '%".$_GET["arama"]."%') ".$where.$orderby);
						...
						$sonuckontrol = 0;
						@$_GET["arama"] = strip_tags($_GET["arama"]);
						$limit = ($sayfa - 1) * $sayfada;
						@$kategoridekiurunsorgu = "SELECT * FROM urunler WHERE (isim LIKE '%".$_GET["arama"]."%' or aciklama LIKE '%".$_GET["arama"]."%' or fiyat LIKE '%".$_GET["arama"]."%' or etiketler LIKE '%".$_GET["arama"]."%') ".$where.$orderby." LIMIT  ".$limit.", ".$sayfada;
						$_GET["arama"] = replace_tr($_GET["arama"]);
						@$kategoridekiurunsorgutr = "SELECT * FROM urunler WHERE (isim LIKE '%".$_GET["arama"]."%' or aciklama LIKE '%".$_GET["arama"]."%' or fiyat LIKE '%".$_GET["arama"]."%' or etiketler LIKE '%".$_GET["arama"]."%') ".$where.$orderby." LIMIT  ".$limit.", ".$sayfada;
						$kategoridekiurunsorgugonder = mysqli_query($db,$kategoridekiurunsorgu);
						$kategoridekiurunsorgugondertr = mysqli_query($db,$kategoridekiurunsorgutr);
					...
					<div class="col-xl-4 col-md-6 grid-item">
						<div class="product" data-price="<?php echo $fiyat_dizi[0]; ?>">
				...
				<?php if($sonuckontrol == 1){ ?>
				<div class="row products_row products_container grid">
					<?php
					if(@mysqli_num_rows($kategoridekiurunsorgugondertr) < 1){
					...
					while(@$kategoridekiurunsorgusatir = mysqli_fetch_array($kategoridekiurunsorgugondertr)){
					if($kategoridekiurunsorgusatir["stok"] > $ayarlarsatir["bitikstok"]){
					$urunresimgetirsorgu = "SELECT * FROM resimler where sahip = ".$kategoridekiurunsorgusatir["id"]." ORDER BY id ASC LIMIT 1 ";
					$urunresimgetirsorgugonder = mysqli_query($db,$urunresimgetirsorgu);
					if(mysqli_num_rows($urunresimgetirsorgugonder) < 1){
								...
								<?php
                                    for($s = 1; $s <= $toplam_sayfa; $s++) {
									   if($sayfa == $s) { // eğer bulunduğumuz sayfa ise link yapma.
									      echo '<li class="active"><a href="?arama='.$_GET["arama"].'&s=' . $s . '">' . $s . '</a></li>';
									   } else {
									      echo '<li><a href="?arama='.$_GET["arama"].'&s=' . $s . '">' . $s . '</a></li>';
									   }
									}
                                    ?>
							</ul>
